...
int main(int argc, char *argv[]) {
  ...
  printf("\nFinding file /DIRECT0/DIRECT1/DIRECT2/DIRECT3/DIRECT4/DEEPFILE.TXT:\n");
  if (!find_directory_entry(volume, "/DIRECT0/DIRECT1/DIRECT2/DIRECT3/DIRECT4/DEEPFILE.TXT", &entry)) {
    mktime(&entry.ctime); // Update weekday and day of the year
    printf("  File name    : %s\n", entry.filename);
    printf("  Creation time: %s\n", asctime(&entry.ctime));
    printf("  File size    : %u\n", entry.size);
  ...
  printf("\nFinding file /MALADE.TXT:\n");
  if (!find_directory_entry(volume, "/MALADE.TXT", &entry)) {
    mktime(&entry.ctime); // Update weekday and day of the year
    printf("  File name    : %s\n", entry.filename);
    printf("  Creation time: %s\n", asctime(&entry.ctime));
    printf("  File size    : %u\n", entry.size);
    ...
    printf("\nFinding file /DIRECT0/DIRECT3/FILE.TXT:\n");
    if (!find_directory_entry(volume, "/DIRECT0/DIRECT3/FILE.TXT", &entry)) {
        mktime(&entry.ctime); // Update weekday and day of the year
        printf("  File name    : %s\n", entry.filename);
        printf("  Creation time: %s\n", asctime(&entry.ctime));
        printf("  File size    : %u\n", entry.size);
  ...
  printf("\nFinding file /DIRECT1/DIRECT3/:\n");
  if (!find_directory_entry(volume, "/DIRECT1/DIRECT3/", &entry)) {
    mktime(&entry.ctime); // Update weekday and day of the year
    printf("  File name    : %s\n", entry.filename);
    printf("  Creation time: %s\n", asctime(&entry.ctime));
    printf("  File size    : %u\n", entry.size);
